...
- Route-scoped components live in a `_components/` folder next to the consuming route (e.g. `routes/agents/$id/_components/`). The TanStack Router plugin ignores `_`-prefixed folders, so they aren't picked up as routes. Promote a component back to `src/client/ui/components/` the moment a second consumer appears — otherwise route-local duplicates accumulate.