...
- **Hermeum leverages configuration over env vars.** Write settings into `config.yaml` unless hermes-agent offers no config replacement for the setting (e.g. `SLACK_ALLOWED_USERS`, Slack/Discord bot tokens) — those stay env-side. Secret-bearing fields that do have a config surface are typed as `SecretRefSchema` values (`hermes-config/shared.ts`, optional; e.g. webhook `platforms.webhook.secret`): the config carries a `${VAR_NAME}` reference (any env var name works; conventional names like `API_SERVER_KEY` appear in docs/examples), hermes substitutes it from the agent env at config load, and the actual value lives in a `sensitive: true` env entry — literal secrets in config are rejected by the regex, and the referenced env var is not cross-validated (pairing is documented, not enforced). Non-secret credentials (e.g. Teams `client_id`/`tenant_id`) are typed as regular config strings — users may treat them as sensitive and keep them env-side. Runtime env-vs-config precedence stays per platform as upstream documents it (api-server: env wins; webhook/teams: config wins).
- **Operator-level concerns are not exposed via agent configuration.** Webhook route `filters`/`script` are not typed — they pass through unvalidated. (`toolsets` was promoted to a typed field so the config generator can grant per-route toolsets by task; see `hermes-config/webhook.ts`.)
