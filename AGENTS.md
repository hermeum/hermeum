...
- **Platform secrets use reserved env vars (temporary deviation).** Write behavioral settings into `config.yaml` when hermes-agent offers a config surface, but secret-bearing settings (webhook `WEBHOOK_SECRET`, api-server `API_SERVER_KEY`, Teams `TEAMS_CLIENT_SECRET`) stay env-side as `sensitive: true` env entries — they are not typed in `hermes-config/` and must not be written into `config.yaml`. This is a rollback of the `${VAR_NAME}` SecretRef scheme (PR #171), forced by two upstream bugs: the gateway never expands `${VAR}` under `platforms:`/`gateway:` (refs reach adapters as literal strings — [hermes-agent#119733](https://github.com/NousResearch/hermes-agent/issues/119733)) and `WEBHOOK_SECRET` is silently dropped unless `WEBHOOK_ENABLED` is also truthy, so webhook enablement is env-only too ([hermes-agent#119763](https://github.com/NousResearch/hermes-agent/issues/119763)). Both are recorded as upstream-differs `:::note`s in the docs. On the next hermes-agent upgrade, verify these issues are fixed and consider restoring the SecretRef scheme (config `${VAR}` reference + `sensitive: true` env pairing, webhook config-path enablement). Non-secret credentials (e.g. Teams `client_id`/`tenant_id`) stay typed as regular config strings — users may treat them as sensitive and keep them env-side. Runtime env-vs-config precedence stays per platform as upstream documents it (api-server: env wins; webhook port/teams: config wins).
