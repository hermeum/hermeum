...
- **Platform secrets use reserved env vars.** Behavioral settings go into `config.yaml` when hermes-agent offers a config surface; secret-bearing settings stay env-side as `sensitive: true` env entries — they are not typed in `hermes-config/` and must not be written into `config.yaml`. This is a standing policy, not the `${VAR_NAME}` SecretRef scheme of PR #171 — do not reintroduce it. The one bug-coupled behavior: webhook *enablement* is env-only (`WEBHOOK_ENABLED=true`) because hermes-agent drops `WEBHOOK_SECRET` when the platform is enabled via config.yaml only ([hermes-agent#119763](https://github.com/NousResearch/hermes-agent/issues/119763)); once a hermes-agent release carries the fix, restore config-path enablement with the secret staying env-side.
