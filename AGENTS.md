...
- **Platform secrets use reserved env vars.** Write behavioral settings into `config.yaml` when hermes-agent offers a config surface, but secret-bearing settings (webhook `WEBHOOK_SECRET`, api-server `API_SERVER_KEY`, Teams `TEAMS_CLIENT_SECRET`) stay env-side as `sensitive: true` env entries — they are not typed in `hermes-config/` and must not be written into `config.yaml`. This is a standing policy, not the `${VAR_NAME}` SecretRef scheme of PR #171 — do not reintroduce it. The one bug-coupled behavior: webhook *enablement* is env-only (`WEBHOOK_ENABLED=true`) because hermes-agent drops `WEBHOOK_SECRET` unless it is also truthy even when the platform is enabled via config.yaml ([hermes-agent#119763](https://github.com/NousResearch/hermes-agent/issues/119763)); once a hermes-agent release carries the fix, restore config-path enablement (`platforms.webhook.enabled` wins, `WEBHOOK_ENABLED` env fallback) with the secret staying env-side. Non-secret credentials (e.g. Teams `client_id`/`tenant_id`) stay typed as regular config strings — users may treat them as sensitive and keep them env-side. Runtime env-vs-config precedence stays per platform as upstream documents it (api-server: env wins; webhook port/teams: config wins).
